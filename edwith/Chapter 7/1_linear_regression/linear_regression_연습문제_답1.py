...
np.ones(len(raw_X)).reshape(-1,1)[:3]

# 11. raw_X행렬의 0 컬럼에 위의 10.에서 만든 one 행렬 붙여주되 열로(열방향) 붙이고 X이름으로 변수로 저장.
X = np.concatenate((np.ones(len(raw_X)).reshape(-1,1), raw_X), axis=1)
X[:5]
# 12. X의 5줄까지 출력
...
plt.legend(bbox_to_anchor=(1.05,1),loc=2,boarderaxespad=0)
